...
const FALLBACKS: Record<string, (section: SectionKey, subtopic: string) => string> = {
  learn: (section, subtopic) =>
    `Let's break down ${subtopicLabel(section, subtopic)} — this shows up often on the ACT, so getting the core idea down now will pay off.`,
...
export async function POST(req: Request) {
  ...
  const fallback = (FALLBACKS[stage] ?? FALLBACKS.learn)(section, subtopic);

  const { text, source } = await getMrKimReply(prompt, fallback, { signal: req.signal });
